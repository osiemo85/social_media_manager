...
export function ConnectionsPage() {
  ...
  return <>
    ...
    <p>Every source is optional, separately configured, expires after 90 days, and can be disabled here.</p>
    {notice && <p className="notice success" role="status">{notice}</p>}
    {error && <p className="notice error" role="alert">{error}</p>}
    <article className="card"><h2>Current connections</h2>{loading ? <p className="muted">Loading connections…</p> : data.connections.length ? <table><thead><tr><th>Provider</th><th>Account and scopes</th><th>Status / expiry</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{data.connections.map((item) => <tr key={item.provider}><td>{item.provider}</td><td>{item.meta.account && <span>{item.meta.account}<br /></span>}<span className="muted">{item.scopes.join(", ")}</span></td><td>{item.status}<br /><span className="muted">{new Date(item.expires_at).toLocaleString()}</span></td><td><button className="danger small" onClick={() => disconnect(item.provider)}>Disconnect &amp; purge</button></td></tr>)}</tbody></table> : <p className="muted">Nothing connected — manual mode is still fully available.</p>}</article>
    ...
    <div className="connection-grid">
      ...
      <article className="card"><h2>GitHub (read-only)</h2><p>Fetch recent repository activity. GitHub consent is independent of Google.</p><button className="secondary" onClick={() => startOAuth("github")}>{connection("github")?.status === "active" ? "Reconnect GitHub" : "Connect GitHub"}</button></article>
    </div>
