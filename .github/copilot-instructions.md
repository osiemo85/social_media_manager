# LinkedIn Social Presence Agent — Instructions

## 1. Purpose

This agent helps developers build a consistent LinkedIn presence by turning
real signals from their work — commits, PRs, releases, talks, calendar
events, docs, emails — into LinkedIn posts. It is multi-user, multi-tenant,
and runs either **locally** (single developer, full data control) or **in
the cloud** (multi-user, managed OAuth + storage).

**Multi-platform publishing**: posting goes through the
[Upload-Post](https://www.upload-post.com) unified API, which supports 22
platforms (LinkedIn, X/Twitter, Threads, Bluesky, Facebook, Reddit,
Telegram, Discord, Mastodon, Pinterest, and more) via a single API call.
LinkedIn is the primary target, but a user can consent per-platform and any
approved draft can fan out to any combination of their connected platforms.

Core principle: **the agent never reads, stores, or posts data without
explicit, scoped, revocable user consent.** Every data source is opt-in,
every connection is a separate consent record, and publishing to LinkedIn is
controlled entirely by a per-user **Publishing Mode** (§5). Users who don't
want to grant access to any account can still use the agent fully via
**Manual Input Mode** (§6).
...
## 4. Consent & Access Model

### 4.1 Principles
1. **Explicit opt-in per source** — nothing is connected by default.
2. **Least privilege scopes** — request the narrowest OAuth scope that
   satisfies the use case (e.g., a Drive folder-picker scope, not full
   Drive read).
3. **Time-boxed consent** — grants expire after N days (default 90) and
   require re-confirmation, even if the underlying OAuth token is still
   technically valid.
4. **Transparent audit trail** — every grant, scope, and revocation is
   recorded in an append-only consent ledger visible to the user
   (`agent audit`).
5. **Human-in-the-loop by default** — publishing to LinkedIn requires
   explicit approval unless the user has separately opted into
   Auto-Publish Mode (§5.2).
6. **Data minimization & TTL** — raw ingested content is kept only as long
   as needed to draft a post (default 30 days), then purged. Only the
   final draft/post + citation metadata may be retained longer.
7. **Right to delete** — a single "disconnect & purge" action per source
   removes its tokens, cached content, and derived signals.
...
```json
{
  ...
  "status": "active",
  "granted_via": "oauth_consent_screen",
  "revocable_url": "/consent/google_drive/revoke",
...
- **App**: Flask app in `web/` — register/login (hashed passwords, session
  cookies), dashboard, manual draft + file upload, connections & consent
  pages, review queue, settings (mode + schedule), audit log.
...
- **Scheduling**: a background scheduler thread checks each user's cadence
  ...
  affects others.
- **Run**: `.venv/bin/python web/app.py` (dev) or
  `.venv/bin/waitress-serve --port=8080 web.app:app`. Expose over the
  internet only behind HTTPS (reverse proxy such as nginx/Caddy, or a
  tunnel for demos).
...
### 8.3 Shared core
CLI and web share identical connector/drafting/consent/routing/scheduler
logic (`smm/` package); only the entry point and the user-context selection
differ, so business logic is written once and served both ways.
...
social_media_manager/
  ...
  agent                     # CLI entry point (./agent --help)
  requirements.txt
  .env.example
  smm/
    config.py               # paths, env, per-user data contexts, defaults
    storage.py              # SQLite: consent, ledger, signals, drafts, settings
    vault.py                # Fernet-encrypted token vault (KMS-swappable)
    consent.py              # grant/require/revoke + append-only audit ledger
    drafting.py             # LLM drafting (OpenAI) with template fallback
    router.py               # Review vs Auto-Publish routing + guardrails (§5)
    publisher.py            # Upload-Post multi-platform publish (consent-gated)
    scheduler.py            # cadence (runs/day), idempotent pipeline runs (§5.4)
    cli.py                  # connect/ingest/draft/review/publish/mode/schedule/daemon/audit
    connectors/
      manual.py             # hints / file upload — no consent needed (§6)
      github.py             # read-only recent activity (optional PAT)
      filesystem.py         # allowlisted-path recent-files scanner (CLI only)
  web/
    app.py                  # Flask multi-user app + background scheduler thread
    templates/              # dashboard, connections, review, settings, audit
  tests/                     # (phase 2)
```
...
- Consent module: encrypted vault, per-platform scopes, TTL, revoke+purge,
  append-only audit ledger (`./agent audit` / web Audit page).
